        ...
        for (int k = 0; k < Nz_; k++) {
            for (int j = 0; j < Ny_; j++) {
                for (int i = 0; i < Nx_len; i++) {
                    ...
                    pois_const_[count + l * Nd_per_shift] = singularity_removal_const(G2);
                    count++;
                }
            }
        }
...
void ExchangePoissonSolver::setup(const FDGrid& grid, const Lattice& lattice,
    ...
    if (rank == 0) {
        ...
        if (exx_div_flag_ == 1)
            std::printf("EXX auxiliary constant = %.10f\n", const_aux_);
        // Print pois_const diagnostics
        int Nd_per_shift = is_gamma_ ? Ndc_ : Nd_;
        int last_shift = Nkpts_shift_ - 1;
        std::printf("DIAG pois_const[G=0] = %.10e (last shift, gamma=%d)\n",
                    pois_const_[last_shift * Nd_per_shift], is_gamma_ ? 1 : 0);
        double Omega = Jacbdet_;
        double V = Omega * Nkpts_hf_;
        double Rc = std::cbrt(3.0 * V / (4.0 * 3.14159265358979323846));
        std::printf("DIAG Omega=%.6f, V=Omega*Nkpts_hf=%.6f, Rc=%.6f, expected_pois_const_G0=%.10e\n",
                    Omega, V, Rc, 2.0 * 3.14159265358979323846 * Rc * Rc);
        std::printf("DIAG Nd=%d, Nx=%d, Ny=%d, Nz=%d, dV=%.10e\n",
                    Nd_, Nx_, Ny_, Nz_, Omega / Nd_);
    }
}
