...
static void basic_MGGA_variables(int length, const double *rho, const double *normDrho, const double *tau, double **s_dsdn_dsddn, double **alpha_dadn_daddn_dadtau) {
    int i;
...
static void Calculate_scanx(int length, const double *rho, double **s_dsdn_dsddn, double **alpha_dadn_daddn_dadtau, double *epsilonx, double *vx1, double *vx2, double *vx3) {
    // constants for h_x^1
    double k1 = 0.065;
...
static void Calculate_scanc(int length, const double *rho, double **s_dsdn_dsddn, double **alpha_dadn_daddn_dadtau, double *epsilonc, double *vc1, double *vc2, double *vc3) {
...
static void basic_MGSGA_variables_exchange(int length, const double *rho, const double *normDrho, const double *tau, double **s_dsdn_dsddn, double **alpha_dadn_daddn_dadtau) { // this function serves for exchange computation
    int i;
    double threeMPi2_1o3 = pow(3.0*M_PI*M_PI, 1.0/3.0);
    double threeMPi2_2o3 = threeMPi2_1o3*threeMPi2_1o3;
...
static void Calculate_scanx_spin(int length, const double *rho, double **s_dsdn_dsddn, double **alpha_dadn_daddn_dadtau, double *epsilonx, double *vx1, double *vx2, double *vx3) {
    // constants for h_x^1
    double k1 = 0.065;
    double mu_ak = 10.0/81.0;
    double b2 = sqrt(5913.0/405000.0);
    ...
    int i;
    double theRho;
    std::vector<double> epsilonx_updn(2*length);
...
static void basic_MGSGA_variables_correlation(int length, const double *rho, const double *normDrho, const double *tau, double **s_dsdn_dsddn, double **alpha_dadnup_dadndn_daddn_dadtau, double **zeta_dzetadnup_dzetadndn) { // this function serves for exchange computation
    int i;
    double threeMPi2_1o3 = pow(3.0*M_PI*M_PI, 1.0/3.0);
    double threeMPi2_2o3 = threeMPi2_1o3*threeMPi2_1o3;
    double theRho, theNormDrho, theTau;
...
static void Calculate_scanc_spin(int length, const double *rho, double **s_dsdn_dsddn, double **alpha_dadnup_dadndn_daddn_dadtau, double **zeta_dzetadnup_dzetadndn, double *epsilonc, double *vc1, double *vc2, double *vc3) {
...
void scanx(int DMnd, const double *rho, const double *sigma, const double *tau,
    ...
    std::vector<double> s0(DMnd), s1(DMnd), s2(DMnd);
    double *s_dsdn_dsddn[3] = {s0.data(), s1.data(), s2.data()};
    std::vector<double> normDrho(DMnd);
    std::vector<double> a0(DMnd), a1(DMnd), a2(DMnd), a3(DMnd);
    double *alpha_dadn_daddn_dadtau[4] = {a0.data(), a1.data(), a2.data(), a3.data()};

    for (int i = 0; i < DMnd; i++) {
        normDrho[i] = sqrt(sigma[i]);
    }

    basic_MGGA_variables(DMnd, rho, normDrho.data(), tau, s_dsdn_dsddn, alpha_dadn_daddn_dadtau);
...
void scanx_spin(int DMnd, const double *rho, const double *sigma, const double *tau,
                double *ex, double *vx, double *v2x, double *v3x) {
    std::vector<double> s0(DMnd*2), s1(DMnd*2), s2(DMnd*2);
    double *s_dsdn_dsddn_exchange[3] = {s0.data(), s1.data(), s2.data()};
    std::vector<double> normDrho(3*DMnd);
    std::vector<double> a0(DMnd*2), a1(DMnd*2), a2(DMnd*2), a3(DMnd*2);
    double *alpha_dadn_daddn_dadtau_exchange[4] = {a0.data(), a1.data(), a2.data(), a3.data()};

    for (int i = 0; i < 3*DMnd; i++) {
        normDrho[i] = sqrt(sigma[i]);
    }

    basic_MGSGA_variables_exchange(DMnd, rho + DMnd, normDrho.data() + DMnd, tau + DMnd, s_dsdn_dsddn_exchange, alpha_dadn_daddn_dadtau_exchange);
...
void scanc_spin(int DMnd, const double *rho, const double *sigma, const double *tau,
                double *ec, double *vc, double *v2c, double *v3c) {
    std::vector<double> s0(DMnd), s1(DMnd), s2(DMnd);
    double *s_dsdn_dsddn_correlation[3] = {s0.data(), s1.data(), s2.data()};
    std::vector<double> normDrho(3*DMnd);
    std::vector<double> a0(DMnd), a1(DMnd), a2(DMnd), a3(DMnd), a4(DMnd);
    double *alpha_dadnup_dadndn_daddn_dadtau_correlation[5] = {a0.data(), a1.data(), a2.data(), a3.data(), a4.data()};
    std::vector<double> z0(DMnd), z1(DMnd), z2(DMnd);
    double *zeta_dzetadnup_dzetadndn[3] = {z0.data(), z1.data(), z2.data()};

    for (int i = 0; i < 3*DMnd; i++) {
        normDrho[i] = sqrt(sigma[i]);
    }

    basic_MGSGA_variables_correlation(DMnd, rho, normDrho.data(), tau, s_dsdn_dsddn_correlation, alpha_dadnup_dadndn_daddn_dadtau_correlation, zeta_dzetadnup_dzetadndn);
