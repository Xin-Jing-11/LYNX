...
void SCF::compute_Veff(const double* rho, const double* rho_b, const double* Vloc) {
    ...
    // During PBE phase, use full PBE exchange. During Fock phase, use hybrid-scaled exchange.
    XCType xc_for_veff = xc_type_;
    if (is_hybrid(xc_type_) && !in_fock_loop_)
        xc_for_veff = hybrid_base_xc(xc_type_);

    XCFunctional xc;
    xc.setup(xc_for_veff, *domain_, *grid_, gradient_, halo_,
             params_.exx_params.hyb_range_fock, params_.exx_params.exx_frac);
...
double SCF::run(Wavefunction& wfn,
    ...
    for (int scf_iter = 0; scf_iter < params_.max_iter; ++scf_iter) {
                ...
                for (int s = 0; s < Nspin_local; ++s) {
                    ...
                    for (int k = 0; k < Nkpts; ++k) {
                        ...
                        if (is_kpt_) {
                            ...
                            // Set EXX context for this spin/kpt
        ...
        {
            // SPARC uses TOL_SCF_INIT (1e-3) for PBE phase of hybrid functionals
            double tol_scf = (exx_ && is_hybrid(xc_type_)) ? std::max(params_.tol, 1e-3) : params_.tol;
            if (scf_iter >= params_.min_iter && scf_error < tol_scf) {
                converged_ = true;
                if (rank_world == 0) {
    ...
    if (exx_ && is_hybrid(xc_type_)) {
        ...
        const auto& exx_p = params_.exx_params;
