...
// ============================================================
// cuBLAS / CUDA runtime thin wrappers (no algorithm logic)
// ============================================================

double Mixer::mixer_dot_gpu(const double* d_a, const double* d_b, int N) {
    auto& ctx = gpu::GPUContext::instance();
    double result = 0.0;
    cublasDdot(ctx.cublas, N, d_a, 1, d_b, 1, &result);
    return result;
}

void Mixer::mixer_axpy_gpu(double alpha, const double* d_x, double* d_y, int N) {
    auto& ctx = gpu::GPUContext::instance();
    cublasDaxpy(ctx.cublas, N, &alpha, d_x, 1, d_y, 1);
}

void Mixer::mixer_scal_gpu(double alpha, double* d_x, int N) {
    auto& ctx = gpu::GPUContext::instance();
    cublasDscal(ctx.cublas, N, &alpha, d_x, 1);
}

void Mixer::mixer_sync_gpu() {
    cudaStream_t stream = gpu::GPUContext::instance().compute_stream;
    CUDA_CHECK(cudaStreamSynchronize(stream));
}

void Mixer::mixer_get_buffers_gpu(double*& d_fk, double*& d_xkm1, double*& d_R, double*& d_F) {
    auto& ctx = gpu::GPUContext::instance();
    d_fk   = ctx.buf.mix_fk;
    d_xkm1 = ctx.buf.mix_xkm1;
    d_R    = ctx.buf.mix_R;
    d_F    = ctx.buf.mix_F;
}

double* Mixer::mixer_scratch_alloc_gpu(size_t count) {
    auto& ctx = gpu::GPUContext::instance();
    return ctx.scratch_pool.alloc<double>(count);
}

size_t Mixer::mixer_scratch_checkpoint_gpu() {
    auto& ctx = gpu::GPUContext::instance();
    return ctx.scratch_pool.checkpoint();
}

void Mixer::mixer_scratch_restore_gpu(size_t cp) {
    auto& ctx = gpu::GPUContext::instance();
    ctx.scratch_pool.restore(cp);
}

void Mixer::mixer_alloc_fkm1_gpu(int N) {
    if (!d_fkm1_) {
        cudaStream_t stream = gpu::GPUContext::instance().compute_stream;
        CUDA_CHECK(cudaMallocAsync(&d_fkm1_, N * sizeof(double), stream));
        CUDA_CHECK(cudaMemsetAsync(d_fkm1_, 0, N * sizeof(double), stream));
    }
}

void Mixer::mixer_memset_gpu(double* d_ptr, int N) {
    cudaStream_t stream = gpu::GPUContext::instance().compute_stream;
    CUDA_CHECK(cudaMemsetAsync(d_ptr, 0, N * sizeof(double), stream));
}

void Mixer::mixer_upload_gpu(double* d_dst, const double* h_src, int count) {
    cudaStream_t stream = gpu::GPUContext::instance().compute_stream;
    CUDA_CHECK(cudaMemcpyAsync(d_dst, h_src, count * sizeof(double), cudaMemcpyHostToDevice, stream));
}

void Mixer::mixer_download_gpu(double* h_dst, const double* d_src, int count) {
    cudaStream_t stream = gpu::GPUContext::instance().compute_stream;
    CUDA_CHECK(cudaMemcpyAsync(h_dst, d_src, count * sizeof(double), cudaMemcpyDeviceToHost, stream));
}

void Mixer::mixer_upload_int_gpu(int* d_dst, const int* h_src, int count) {
    cudaStream_t stream = gpu::GPUContext::instance().compute_stream;
    CUDA_CHECK(cudaMemcpyAsync(d_dst, h_src, count * sizeof(int), cudaMemcpyHostToDevice, stream));
}
