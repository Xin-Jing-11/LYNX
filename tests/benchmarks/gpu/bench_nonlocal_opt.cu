// Optimized nonlocal projector kernel benchmark
// Key insight: original gather launches only n_atoms blocks (4) — 95% GPU idle.
// Fix: launch one block per (atom, column) to expose parallelism.
...
// ============================================================
// ORIGINAL: One block per atom, inner loop over columns
// Only n_atoms blocks total — terrible GPU utilization
// ============================================================
static constexpr int NL_TILE = 256;
static constexpr int NL_MAX_NP = 32;

__global__ void fused_gather_chitpsi_kernel_orig(
...
// ============================================================
// OPTIMIZED V1: One block per (atom, column)
// n_atoms * ncol blocks — much better GPU utilization
// blockIdx.x = iat, blockIdx.y = column
// ============================================================
__global__ void fused_gather_chitpsi_kernel_v1(
    const double* __restrict__ psi,
    const double* __restrict__ Chi_flat,
    const int* __restrict__ gpos_flat,
    const int* __restrict__ gpos_offsets,
...
// ============================================================
// OPTIMIZED V2: One block per (atom, column), warp-per-projector reduction
// Avoids sequential block-reduce loop over np projectors
// Uses warp shuffle instead of shared memory block reduce for each projector
// ============================================================
    ...
    double dV, int n_atoms)
{
    ...
    // Use registers for accumulation (no shared mem tile for psi)
    // Direct global memory gather — relies on L2 cache for scattered reads
    double dots[NL_MAX_NP];
    for (int jp = 0; jp < np; jp++) dots[jp] = 0.0;

    // Process ndc points with stride
...
// ============================================================
// OPTIMIZED scatter: one block per (atom, column)
// ============================================================
__global__ void fused_chialpha_scatter_kernel_orig(
    ...
    int n_atoms)
{
    ...
    // Load alpha for this column into registers
    extern __shared__ double alpha_sh[];
    if (threadIdx.x < np)
...
int main() {
    ...
    int block_size = 256;
    size_t smem_orig = (NL_TILE + block_size / 32) * sizeof(double);
    size_t smem_v2 = (block_size / 32) * sizeof(double);
    size_t smem_scatter_orig = (size_t)nproj_per_atom * ncol * sizeof(double);
    size_t smem_scatter_v1 = nproj_per_atom * sizeof(double);

    printf("=== Nonlocal Projector: ORIGINAL vs OPTIMIZED ===\n");
    printf("    atoms=%d ndc=%d nproj=%d ncol=%d (NREPS=%d)\n\n", n_atoms, ndc_per_atom, nproj_per_atom, ncol, NREPS);
    ...
    auto bench_scatter = [&](const char* name, auto launcher) {
        ...
        CUDA_CHECK(cudaEventSynchronize(stop));
        float ms; cudaEventElapsedTime(&ms, start, stop);
        printf("  scatter %-11s: %8.4f ms\n", name, ms / NREPS);
    };

    // GATHER benchmarks
    bench_gather("orig", [&]() {
        fused_gather_chitpsi_kernel_orig<<<n_atoms, block_size, smem_orig>>>(
    ...
    bench_gather("v1-2D", [&]() {
        dim3 grid(n_atoms, ncol);
        fused_gather_chitpsi_kernel_v1<<<grid, block_size, smem_orig>>>(
            d_psi, d_Chi_flat, d_gpos_flat, d_gpos_offsets, d_chi_offsets,
            d_ndc_arr, d_nproj_arr, d_IP_displ, d_alpha,
            Nd, ncol, ncol, 0, dV, n_atoms);
    });

    bench_gather("v2-nosmem", [&]() {
        dim3 grid(n_atoms, ncol);
        fused_gather_chitpsi_kernel_v2<<<grid, block_size, smem_v2>>>(
            d_psi, d_Chi_flat, d_gpos_flat, d_gpos_offsets, d_chi_offsets,
            d_ndc_arr, d_nproj_arr, d_IP_displ, d_alpha,
            Nd, ncol, ncol, 0, dV, n_atoms);
    });
    ...
    // SCATTER benchmarks
    bench_scatter("orig", [&]() {
        fused_chialpha_scatter_kernel_orig<<<n_atoms, block_size, smem_scatter_orig>>>(
    ...
    bench_scatter("v1-2D", [&]() {
        dim3 grid(n_atoms, ncol);
        fused_chialpha_scatter_kernel_v1<<<grid, block_size, smem_scatter_v1>>>(
            d_Hpsi, d_Chi_flat, d_gpos_flat, d_gpos_offsets, d_chi_offsets,
            d_ndc_arr, d_nproj_arr, d_IP_displ, d_alpha,
            Nd, ncol, ncol, 0, n_atoms);
    });
